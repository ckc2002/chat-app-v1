  ...
  return (
    ...
    max-sm:flex-col backdrop-blur-2xl'>
      ...
      <form onSubmit={onSubmitHandler} className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
...

        {
          currState === "Sign Up" && !isDataSubmitted && (
            <input type="text" className='p-2 border border-gray-500 rounded-md focus:outline-none'
              placeholder='Full Name' onChange={(e) => setFullName(e.target.value)} value={fullName} required />
          )
        }

        {
          !isDataSubmitted && (
            <>
              ...
              <input type="password" onChange={(e) => setPassword(e.target.value)} value={password} placeholder='Passowrd'
                className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500' />
            </>
          )
        }
        ...
        {
          currState === "Sign Up" && isDataSubmitted && (
            <textarea value={bio} rows={4} className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2
             focus:ring-indigo-500' placeholder='provide a sort bio...' onChange={(e) => setBio(e.target.value)}></textarea>
          )
        }
